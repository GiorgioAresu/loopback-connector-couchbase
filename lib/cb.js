...
CouchbaseDB.prototype.save = function (model, data, callback) {
	...
	var myBucket = self.myCluster.openBucket(self.settings.bucket,function(err){
		...
		updateOneDoc(self,myBucket,data.id,model,data,callback);
	});
};
...
CouchbaseDB.prototype.destroy = function destroy(model, id, callback) {
	...
	debug("CouchbaseDB.prototype.destroy() : " + JSON.stringify([model, parsedId]));

	removeOneDoc(parsedId,self,function(err, result){
		if(err){
			callback(err,null);
			return;
		}
...
CouchbaseDB.prototype.destroyAll = function destroyAll(model, filter,callback) {
	...
	if (isRequestedByIDOnly(filterobj) && isRequestedBySingleValue(filterobj[constants.KEYWORD_CONVENTION_DOCID])) {
		var value = filterobj[constants.KEYWORD_CONVENTION_DOCID];
		var id = extractId(value);
		debug('CouchbaseDB.prototype.destroyAll() : remove this doc by id : ' + id);
		this.destroy(model, id, callbackfn);
		return;
	};
	var self = this;
	var clauseBuilderIndex = new clauseBuilderClass(); 
...
var isRequestedBySingleValue = function(filter) {
  if (Object.prototype.toString.call(filter) === '[Object object]') {
    return filter.inq !== null && filter.inq.length === 1;
  }
  return true;
};
